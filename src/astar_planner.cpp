...
void AStarPlanner::initialize(std::string name, costmap_2d::Costmap2DROS *costmap_ros)
{
  ...
  if (!initialized_)
  {
    ...
    occupancyGridMap = new bool[mapSize]; // using as closed List
...
int AStarPlanner::getGridSquareIndex(float x, float y)
{
  ...
  float newX = x / (resolution); //cell number since x.
  float newY = y / (resolution);

  gridSquare = calculateGridSquareInd(newY, newX);
...
float AStarPlanner::calculateHScore(int gridSquareIndex, int goalGridSquare)
{
  ...
  int x2 = getGridSquareRowInd(gridSquareIndex);
  int y2 = getGridSquareColInd(gridSquareIndex);
...
**/
int AStarPlanner::getGridSquareRowInd(int index) //get the row index from gridSquare index
{
  return index / width;
}
...
**/
int AStarPlanner::getGridSquareColInd(int index) //get column index from gridSquare index
{
  return index % width;
}
